...
export class Ewma {
  ...
  constructor(
    ...
    invariant(!Number.isNaN(halfLifeMs), "halfLifeMs can not be NaN");
    invariant(typeof initialValue === "number", "initialValue is not a number");
    invariant(!Number.isNaN(initialValue), "initialValue can not be NaN");
  ...
  insert(x: number) {
    ...
    invariant(!Number.isNaN(x), "x can not be NaN");
